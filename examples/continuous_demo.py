...
dt_max = 2.
T = 100.
network_hypers = {'C': 1, 'kappa': 1., 'c': np.zeros(K, dtype=np.int), 'p': np.ones((1,1)), 'v': 10.}
...
S_dt,_ = dt_model.generate(T=int(np.ceil(T/dt)),
                             keep=False)

print "DT LL: ", dt_model.heldout_log_likelihood(S_dt)

# Convert S_test to continuous time
S_ct = np.concatenate([ibincount(S) for S in S_dt.T]).astype(float)
S_ct += np.random.rand(*S_ct.shape)
...
ct_model = pyhawkes.models.ContinuousTimeNetworkHawkesModel(K, dt_max=dt_max)
